...
MsgKplugDetected=WARNING - KoikPlugins/ksPlug was detected and will be removed if you start installation to avoid potential compatibility issues (settings will not be removed).%n%nPlease follow KoikPlugins manual on how to install it again after patching is done.
...
MsgExeNotFound=WARNING - Koikatsu Sunshine files were not found in selected directory. This patch has to be installed directly to the main Koikatsu game directory in order to work properly.%n%nAre you sure that this directory is correct?
...
RunGame=&Run Koikatsu Sunshine
RunWiki=Open Koikatsu Sunshine wiki (FAQ, English manual)
...
jp.MsgKplugDetected=警告 - 互換性の問題を回避するためにインストールを開始すると、ksPlugは削除されます（設定は削除されません）。%n%n後でそれをインストールする方法についてはksPlugマニュアルに従ってください
...
jp.MsgExeNotFound=警告 - 「コイカツ！サンシャイン」が選択されたディレクトリに見つかりませんでした。 正しく動作させるためには、このパッチをKoikatsuのメインゲームディレクトリに直接インストールする必要があります。%n%nこのディレクトリが正しいことを確認していますか？
...
jp.RunGame=ゲームを始める
jp.RunWiki=オープン英語コイカツ！サンシャインのwiki（FAQ、マニュアル）
